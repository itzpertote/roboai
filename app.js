...
if (currentModel === "robo-1.0-local") {

  reply =
    clientDemoReply(message);

} else if (
  currentModel === "robo-1.1-mini"
) {

  reply =
    await roboMiniReply(message);
...
let currentModel = "robo-1.0-local";
let loggedIn = false;
...
const miniModelButton =
  document.getElementById("miniModelButton");

const miniModelText =
  document.getElementById("miniModelText");
...
loginSubmit?.addEventListener("click", async () => {
  ...
  if (
    ...
    miniModelButton.classList.remove("locked");

    miniModelText.textContent =
      language === "tr"
...
async function roboMiniReply(message) {
  ...
  try {
    ...
    const response = await fetch(
      `https://searx.be/search?q=${encodeURIComponent(message)}&format=json`
    );
